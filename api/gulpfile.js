...
const copyNodeJSCodeTask = () => {
  ...
  src('build/index.js').pipe(dest(`${paths.server_source_dest}`));
...
const addEngineToPackage = () =>
  exec(
    `yarn json -I -f ${paths.prod_build}/package.json -e "this.engines = { 'node': '16.0.0' }"`,
  );
const updatePackage = () =>
  exec(
    `yarn json -I -f ${paths.prod_build}/package.json -e "this.scripts.start = 'NODE_ENV=production node -r dotenv/config ./dist/src/index.js'"`,
  );
